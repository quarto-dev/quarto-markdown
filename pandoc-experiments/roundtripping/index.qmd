...
In practice we will sometimes say that some document "is" an AST as well.
In this context, we mean the result of `pandoc -t native` or some input to `pandoc -f native`.
We try to keep the two different meanings clear, and to never intentionally refer to the same document as both a document that will be read as (eg) `markdown` and an AST.
...
We have an analogous definition:
...
and so roundtripping will fail, and so the AST cannot be canonical. (This can be verified by inspecting the Pandoc source code)

Finally, `pandoc` writers ignore `RawBlock` and `RawInline` nodes of formats that are not their own, immediately failing
the case where the `format` field of these nodes doesn't match the format relatively to which we're making the statement. (This, once again, is verified by inspecting the Pandoc source code)
