...
fn test_no_violations_in_correct_file() {
    ...
    // File with properly closed strikeout
    fs::write(
        &test_file,
        r#"This has ~~deleted text~~ properly closed.
"#,
    )
    .unwrap();

    let registry = RuleRegistry::new().unwrap();
    let rule = registry.get("q-2-18").unwrap();
...
fn test_detects_single_violation() {
    ...
    fs::write(&test_file, "~~deleted text\n").unwrap();

    let registry = RuleRegistry::new().unwrap();
    let rule = registry.get("q-2-18").unwrap();

    let results = rule.check(&test_file, false).unwrap();
    assert_eq!(results.len(), 1, "Should detect one Q-2-18 violation");
...
fn test_converts_single_violation() {
    ...
    assert_eq!(
        converted, "~~deleted text~~\n",
        "Should add closing strikeout marks"
    );
}
