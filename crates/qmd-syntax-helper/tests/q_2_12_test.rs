...
fn test_no_violations_in_correct_file() {
    ...
    // File with properly closed star emphasis
    fs::write(
        &test_file,
        r#"This has *properly closed emphasis* in it.
"#,
    )
    .unwrap();
...
fn test_detects_single_violation() {
    ...
    fs::write(&test_file, "*Unclosed emphasis\n").unwrap();

    let registry = RuleRegistry::new().unwrap();
    let rule = registry.get("q-2-12").unwrap();

    let results = rule.check(&test_file, false).unwrap();
    assert_eq!(results.len(), 1, "Should detect one Q-2-12 violation");
...
fn test_converts_single_violation() {
    ...
    let original = "*Unclosed emphasis\n";
    ...
    assert_eq!(
        converted, "*Unclosed emphasis*\n",
        "Should add closing star"
    );
}
...
fn test_in_place_conversion() {
    ...
    assert_eq!(content, "*Unclosed emphasis*\n");
}
...
fn test_no_changes_when_all_correct() {
    ...
    fs::write(&test_file, "*Properly closed emphasis*\n").unwrap();
...
fn test_complex_case_with_content() {
    ...
    fs::write(
        &test_file,
        "*This is a long sentence with lots of content and an unclosed emphasis\n",
    )
    .unwrap();
