...
fn test_no_violations_in_correct_file() {
    ...
    // File with properly closed editorial insert
    fs::write(
        &test_file,
        r#"This has [++properly closed insert++] in it.
"#,
    )
    .unwrap();

    let registry = RuleRegistry::new().unwrap();
    let rule = registry.get("q-2-19").unwrap();
...
fn test_detects_single_violation() {
    ...
    fs::write(&test_file, "[++Unclosed insert\n").unwrap();

    let registry = RuleRegistry::new().unwrap();
    let rule = registry.get("q-2-19").unwrap();

    let results = rule.check(&test_file, false).unwrap();
    assert_eq!(results.len(), 1, "Should detect one Q-2-19 violation");
