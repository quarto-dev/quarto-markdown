...
fn test_no_violations_in_correct_file() {
    ...
    // File with properly closed editorial delete
    fs::write(
        &test_file,
        r#"This has [--properly closed delete--] in it.
"#,
    )
    .unwrap();

    let registry = RuleRegistry::new().unwrap();
    let rule = registry.get("q-2-20").unwrap();
...
fn test_detects_single_violation() {
    ...
    fs::write(&test_file, "[--Unclosed delete\n").unwrap();

    let registry = RuleRegistry::new().unwrap();
    let rule = registry.get("q-2-20").unwrap();

    let results = rule.check(&test_file, false).unwrap();
    assert_eq!(results.len(), 1, "Should detect one Q-2-20 violation");
