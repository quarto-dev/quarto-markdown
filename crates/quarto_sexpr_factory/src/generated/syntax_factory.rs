        ...
        match kind {
            ...
            SEXPR_LIST => {
                ...
                if let Some(element) = &current_element {
                    if SexprItemList::can_cast(element.kind()) {
                        slots.mark_present();
                ...
                slots.into_node(SEXPR_LIST, children)
            }
            SEXPR_LIST_ITEM => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<1usize> = RawNodeSlots::default();
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        SEXPR_LIST_ITEM.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(SEXPR_LIST_ITEM, children)
            }
            SEXPR_LIST_VALUE => {
                let mut elements = (&children).into_iter();
            ...
            SEXPR_ITEM_LIST => Self::make_node_list_syntax(kind, children, SexprListItem::can_cast),
